...
var configUseCmd = &cobra.Command{
	Use:   "use",
	Short: "Change current environment",
	Long:  `Change current environment.`,
	Run:   configUseRun,
}

func configUseRun(cmd *cobra.Command, args []string) {
	if len(args) != 1 {
		fmt.Printf("Args expects to have 1, but it have %d\n", len(args))
		os.Exit(0)
	}
	config := Config.Load()
	if err := config.UpdateCurrent(args[0]); err != nil {
